...
import { MILLISECONDS_PER_SECOND, POMODORO_DEFAULT_STATE, POMODORO_STATUS } from "~constants";
import { initialSyncWithLocalStorage, setPomodoroToLocalStorage, skipPomodoroMode } from "~helpers";
...
export const usePomodoroStore = create<PomodoroStore>((set) => ({
    ...
    toggleSounds: () =>
        set((store) => {
            ...
            setPomodoroToLocalStorage("settings", updatedSettings);
    ...
    resetSettings: () =>
        set((store) => {
            const updatedSettings = POMODORO_DEFAULT_STATE.settings;
            setPomodoroToLocalStorage("settings", updatedSettings);
                ...
                state: {
                    ...
                    session: {
                        ...
                        duration:
                            store.state.session.status === "idle" ?
                                {
                                    total: POMODORO_DEFAULT_STATE.settings.durations[store.state.session.mode],
                                    remaining: POMODORO_DEFAULT_STATE.settings.durations[store.state.session.mode]
                                }
                            :   store.state.session.duration
                    }
                }
        ...
        set((store) => {
            ...
            if (isTimeUp) {
                ...
                if (isFocusMode) {
                    ...
                    setPomodoroToLocalStorage("stats", updatedStats);
                }
            }
    ...
    toggleStatus: (playSound) =>
        set((store) => {
            const updatedStats = POMODORO_STATUS.transition.status[store.state.session.status];
